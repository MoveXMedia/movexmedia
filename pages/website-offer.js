...
export default function WebsitePromo() {
  return (
    <>
      <Head>
        <title>Physio Websites — Special Offer | MoveX Media</title>
        <meta name="description" content="Limited time offer: professional physiotherapy websites from $1,500. Fast, SEO-optimised, and built to convert patients." />
        ...
        <style>{`
          ...
          * { box-sizing: border-box; margin: 0; padding: 0; }
          ...
          .fade-up { opacity: 0; transform: translateY(20px); animation: fadeUp 0.6s forwards; }
      ...
      <section style={{ position: 'relative', padding: '7rem 1.5rem 5rem', textAlign: 'center', overflow: 'hidden', background: 'linear-gradient(160deg, #060d18 0%, #0a1628 60%, #060d18 100%)' }}>
        ...
        <div style={{ position: 'relative', maxWidth: '860px', margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap', marginBottom: '2.5rem' }}>
            {[
              ...
              <div key={pkg.label} style={{
                background: pkg.highlight ? 'linear-gradient(135deg, rgba(91,196,245,0.15), rgba(91,196,245,0.05))' : 'rgba(255,255,255,0.04)',
                border: `1px solid ${pkg.highlight ? 'rgba(91,196,245,0.5)' : 'rgba(255,255,255,0.1)'}`,
                borderRadius: '12px', padding: '1.25rem 2rem', minWidth: '200px',
              }}>
          ...
          <a href={CLICKFUNNELS_URL} className="lp-btn lp-btn-lg pulse">
            Claim Your Spot →
          </a>
          <p style={{ color: 'rgba(255,255,255,0.25)', fontSize: '0.78rem', marginTop: '1rem', fontWeight: 300 }}>
            Limited availability · No lock-in contracts
          </p>
        </div>
      </section>
      ...
      <div style={{ background: 'rgba(91,196,245,0.06)', borderTop: '1px solid rgba(91,196,245,0.12)', borderBottom: '1px solid rgba(91,196,245,0.12)', padding: '1.25rem 1.5rem' }}>
        <div style={{ maxWidth: '900px', margin: '0 auto', display: 'flex', gap: '2.5rem', justifyContent: 'center', flexWrap: 'wrap', alignItems: 'center' }}>
          {[
            ...
            { num: '#1', label: 'Google Rankings' },
            { num: '4.7×', label: 'Avg. Client LTV' },
            { num: '48hr', label: 'Launch Timeline' },
          ].map(s => (
            <div key={s.label} style={{ textAlign: 'center' }}>
              <div style={{ fontWeight: 900, fontSize: '1.6rem', color: '#5bc4f5', lineHeight: 1, fontFamily: "'Poppins', sans-serif" }}>{s.num}</div>
      ...
      <section style={{ padding: '6rem 1.5rem', maxWidth: '1100px', margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: '3.5rem' }}>
          ...
          <h2 style={{ fontWeight: 800, fontSize: 'clamp(1.8rem, 4vw, 2.8rem)', lineHeight: 1.2, marginTop: '1rem', marginBottom: '0.75rem' }}>
            Real Sites. Real Clinics.
          </h2>
          <div className="divider" />
        </div>

        {/* Move Physio — full width feature */}
        <div style={{ position: 'relative', borderRadius: '20px', overflow: 'hidden', border: '1px solid rgba(91,196,245,0.2)', marginBottom: '1.5rem', background: '#0a1628' }}>
          <Image
            src="/work-move-physio.png"
            alt="Move Physiotherapy Website"
            width={1200}
            height={680}
          ...
          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'linear-gradient(to top, rgba(6,13,24,0.95), transparent)', padding: '2.5rem 2rem 1.75rem' }}>
            <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
              ...
              <a href="https://movephysio.com.au" target="_blank" rel="noopener noreferrer" style={{ display: 'inline-block', background: 'rgba(91,196,245,0.15)', border: '1px solid rgba(91,196,245,0.4)', color: '#5bc4f5', padding: '0.6rem 1.4rem', borderRadius: '6px', fontWeight: 600, fontSize: '0.78rem', textDecoration: 'none', whiteSpace: 'nowrap' }}>
                View Live Site →
              </a>
            </div>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }} className="portfolio-grid-2">
          {[
            { src: '/work-hexa-1.png', alt: 'Hexa Health Hub website', title: 'Hexa Health Hub', desc: 'Premium allied health brand' },
            { src: '/work-hexa-2.png', alt: 'Hexa Health Hub interior page', title: 'Hexa — Service Pages', desc: 'Condition-specific landing pages' },
          ].map(item => (
            <div key={item.title} style={{ borderRadius: '16px', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.08)', background: '#0a1628', position: 'relative' }}>
        ...
        <style>{`
          @media (max-width: 640px) { .portfolio-grid-2 { grid-template-columns: 1fr !important; } }
        `}</style>
      </section>

      {/* ─── PAGE SPEED ─── */}
      <section style={{ padding: '6rem 1.5rem', background: 'linear-gradient(160deg, #080f1a 0%, #0d1f35 100%)' }}>
        <div style={{ maxWidth: '900px', margin: '0 auto', textAlign: 'center' }}>
          <span className="tag">Performance</span>
          <h2 style={{ fontWeight: 800, fontSize: 'clamp(1.8rem, 4vw, 2.8rem)', lineHeight: 1.2, margin: '1rem 0 0.75rem' }}>
            Perfect PageSpeed Scores.<br />
            <span style={{ color: '#5bc4f5' }}>Not Just Good — 100.</span>
          </h2>
          <div className="divider" />
          <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.95rem', lineHeight: 1.8, maxWidth: '560px', margin: '1.5rem auto 3rem', fontWeight: 300 }}>
            Most physio websites score below 50. We build on modern frameworks with CDN delivery, image optimisation, and Core Web Vitals compliance baked in from day one.
          </p>

          {/* Score displays */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '1.5rem', marginBottom: '2.5rem' }}>
            {[
              { label: 'Performance', score: 100, colour: '#22c55e' },
              { label: 'Accessibility', score: 100, colour: '#22c55e' },
              { label: 'Best Practices', score: 100, colour: '#22c55e' },
              { label: 'SEO', score: 100, colour: '#22c55e' },
            ].map(item => (
              <div key={item.label} style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '16px', padding: '2rem 1rem' }}>
                <svg viewBox="0 0 80 80" style={{ width: '80px', height: '80px', display: 'block', margin: '0 auto 1rem' }}>
                  ...
                  <circle cx="40" cy="40" r="34" fill="none" stroke={item.colour} strokeWidth="7"
                    strokeDasharray="213.6"
                    strokeDashoffset={213.6 - (213.6 * item.score / 100)}
                    strokeLinecap="round"
                    transform="rotate(-90 40 40)"
                  />
                  <text x="40" y="45" textAnchor="middle" fontSize="18" fontWeight="800" fill={item.colour} fontFamily="Poppins, sans-serif">{item.score}</text>
                </svg>
                <div style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.5)', fontWeight: 400 }}>{item.label}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.25)', borderRadius: '12px', padding: '1.25rem 1.75rem', display: 'inline-block' }}>
            <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '0.85rem', fontWeight: 300, margin: 0 }}>
              ⚡ <strong style={{ color: 'white' }}>Fast sites rank higher</strong> — Google rewards speed with better positions. Slow sites lose patients <em>and</em> rankings.
            </p>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: '6rem 1.5rem', maxWidth: '1100px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.25rem' }}>
          {[
            ...
            { icon: '📱', title: 'Mobile-First Design', desc: 'Over 70% of health searches happen on mobile. Your site looks perfect on every screen, from phone to desktop.' },
            { icon: '📅', title: 'Online Booking Integration', desc: 'Connected to Cliniko, Nookal, HotDoc, or PowerDiary so patients can book directly from every page.' },
            { icon: '✏️', title: 'Edit It Yourself — After Launch', desc: 'We build in a simple content editor so you can update text, images, and blog posts without touching any code. No web developer needed.' },
            { icon: '📝', title: 'Copy Written For You', desc: 'We write all your website copy — no blank pages to fill in. AHPRA-compliant and conversion-focused out of the box.' },
            { icon: '🎨', title: 'Custom Brand Design', desc: 'Not a template — a proper custom design aligned to your clinic colours, logo, and personality. Looks like no other site.' },
            { icon: '🏥', title: 'Service & Condition Pages', desc: 'Individual pages for each service (physio, dry needling, pilates, etc.) that target patients searching for exactly what you offer.' },
            { icon: '📊', title: 'Google Analytics + Tracking', desc: 'Full conversion tracking from day one — see how many people visit, where they come from, and how many book an appointment.' },
          ].map(f => (
      ...
      <section style={{ padding: '5rem 1.5rem', background: 'rgba(8,15,26,0.8)', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
        <div style={{ maxWidth: '900px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4rem', alignItems: 'center' }} className="edit-grid">
          <div>
            ...
            <h2 style={{ fontWeight: 800, fontSize: 'clamp(1.6rem, 3vw, 2.4rem)', lineHeight: 1.2, marginBottom: '1.25rem' }}>
              You own it.<br />
              <span style={{ color: '#5bc4f5' }}>Edit it yourself.</span>
            </h2>
            <p style={{ color: 'rgba(255,255,255,0.55)', fontSize: '0.95rem', lineHeight: 1.8, fontWeight: 300, marginBottom: '1.5rem' }}>
              Every site we build comes with a simple editing system so you can update your content without needing us — or any developer. Change your hours, add a new service, post a blog article — it's all point-and-click.
            </p>
            <p style={{ color: 'rgba(255,255,255,0.55)', fontSize: '0.95rem', lineHeight: 1.8, fontWeight: 300 }}>
              We also provide a short walkthrough video specific to your site so your reception team can handle day-to-day updates with confidence.
            </p>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {[
              'Update text and images — no code',
              'Add new blog posts yourself',
              'Change business hours and contact info',
              'Add or edit service descriptions',
        ...
        <style>{`
          @media (max-width: 760px) { .edit-grid { grid-template-columns: 1fr !important; gap: 2.5rem !important; } }
        `}</style>
      </section>
      ...
      <section style={{ padding: '6rem 1.5rem', maxWidth: '900px', margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: '3.5rem' }}>
          ...
          <div className="divider" />
          <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.85rem', fontWeight: 300, marginTop: '1rem' }}>
            One-off cost. No monthly fees. No hidden charges.
          </p>
        </div>
          ...
          {[
            {
              ...
              price: '$1,500',
              pages: '3 pages',
              desc: 'A clean, professional online presence for a solo practitioner or new clinic.',
              includes: ['Home, About & Contact pages', 'Online booking integration', 'Mobile-responsive design', 'Basic SEO setup', 'Google Analytics', '30-day support'],
              highlight: false,
            },
            {
              name: 'Growth',
              price: '$2,500',
              pages: '6–10 pages',
              desc: 'Built to rank on Google and convert more of the visitors you\'re already getting.',
              includes: ['Up to 10 pages incl. service pages', 'Custom brand-aligned design', 'Advanced SEO + schema markup', 'Condition/service landing pages', 'Blog setup', 'Conversion tracking', '60-day support'],
              highlight: true,
            },
          ].map(pkg => (
            <div key={pkg.name} style={{
              background: pkg.highlight ? 'linear-gradient(160deg, rgba(15,37,64,0.9), rgba(20,50,80,0.7))' : 'rgba(255,255,255,0.03)',
              border: `1px solid ${pkg.highlight ? 'rgba(91,196,245,0.45)' : 'rgba(255,255,255,0.08)'}`,
              borderRadius: '20px',
              padding: '2.5rem 2rem',
              position: 'relative',
              transform: pkg.highlight ? 'scale(1.02)' : 'scale(1)',
              boxShadow: pkg.highlight ? '0 0 60px rgba(91,196,245,0.1)' : 'none',
            }}>
              {pkg.highlight && (
                <div style={{ position: 'absolute', top: '-12px', left: '50%', transform: 'translateX(-50%)', background: 'linear-gradient(90deg, #5bc4f5, #2a9fd8)', color: '#060d18', fontSize: '0.65rem', fontWeight: 800, padding: '0.3rem 1.2rem', borderRadius: '20px', letterSpacing: '0.08em', whiteSpace: 'nowrap' }}>
                  MOST POPULAR
                </div>
              )}
        ...
        <style>{`
          @media (max-width: 620px) { .pkg-grid { grid-template-columns: 1fr !important; } }
        `}</style>
      </section>
      ...
      <section style={{ padding: '6rem 1.5rem 7rem', textAlign: 'center', background: 'linear-gradient(160deg, #080f1a 0%, #0d1f35 50%, #080f1a 100%)', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', maxWidth: '680px', margin: '0 auto' }}>
          <span className="tag" style={{ marginBottom: '1.5rem', display: 'inline-block' }}>Limited Spots Available</span>
          <h2 style={{ fontWeight: 900, fontSize: 'clamp(2rem, 5vw, 3.2rem)', lineHeight: 1.15, marginBottom: '1.25rem', letterSpacing: '-0.02em' }}>
            Ready to stop losing patients<br />to a slow, outdated website?
          </h2>
          <p style={{ color: 'rgba(255,255,255,0.55)', fontSize: '1rem', lineHeight: 1.8, fontWeight: 300, marginBottom: '2.5rem' }}>
            We take on a limited number of builds each month to ensure every site gets the attention it deserves. Claim your spot now.
          </p>
          <a href={CLICKFUNNELS_URL} className="lp-btn lp-btn-lg pulse">
            Claim Your Spot Now →
          </a>
